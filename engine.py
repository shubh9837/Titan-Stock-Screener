...
def update_daily_analysis(ticker_list):
    ...
    packet_size = 50 # Smaller packets are more reliable for metadata
        ...
        try:
            ...
            for f_sym in packet:
                try:
                    ...
                    cmp, rsi = df['Close'].iloc[-1], df['RSI'].iloc[-1]
                    ...
                    info = t_obj.info
                    # Try multiple keys to find the industry
                    industry = info.get('sector', info.get('industry', info.get('industryDisp', 'General')))
