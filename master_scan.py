...
print("Initiating God-Mode Master Scan...")
...
for t in symbols: 
    ...
    try:
        ...
        df.ta.macd(fast=12, slow=26, signal=9, append=True) # Added MACD
        ...
        # Volume Dry-up: Were the last 3 days very quiet before today?
        vol_dry_up = False
        if len(df) > 5:
            last_3_vol_avg = df['Volume'].iloc[-4:-1].mean()
            if last_3_vol_avg < avg_vol and rvol > 1.5: vol_dry_up = True

        # Pre-Breakout Math: Squeeze + Near Resistance + MACD Momentum
        ...
        if vol_dry_up: score += 20 # Massive points for smart money volume footprint
        elif rvol > 1.5: score += 10
        
        if is_pre_breakout: score += 30 # Immediate Alpha Status
        ...
        batch_results.append({
            ...
            "SCORE": max(0, min(100, score)), # Floor at 0, Cap at 100
            "RSI": round(rsi, 2),
        ...
        if len(batch_results) >= BATCH_SIZE:
            ...
            print(f"📦 [STREAM] Scanned: {processed_count}/{len(symbols)} | Validated: {success_count}")
            batch_results = [] 
            
        time.sleep(0.3) 
    except Exception as e: continue

if batch_results: supabase.table('market_scans').upsert(batch_results).execute()
print(f"✅ Full Universe Stream Complete. Validated: {success_count}")
